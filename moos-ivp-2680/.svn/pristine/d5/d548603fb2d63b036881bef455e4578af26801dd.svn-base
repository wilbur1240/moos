...
std::string GradeFrontEstimate::handleSensingReport(const string& request)
{
  ...
  for(i=0; i<vsize; i++) {
    ...
    if(param == "vname")
      m_festimate.r_vname = value;
    else if (param == "offset")
      m_festimate.r_offset = atof(value.c_str());
    else if (param == "angle")
      m_festimate.r_angle = atof(value.c_str());
    else if (param == "amplitude")
      m_festimate.r_amplitude = atof(value.c_str());
    else if (param == "period")
      m_festimate.r_period = atof(value.c_str());
    else if (param == "wavelength")
      m_festimate.r_wavelength = atof(value.c_str());
    else if (param == "alpha")

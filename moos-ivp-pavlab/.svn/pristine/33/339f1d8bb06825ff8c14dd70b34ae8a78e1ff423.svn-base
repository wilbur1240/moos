...
for ARGI; do
	...
	echo "                                               "
	echo "Options (custom):                              "
	echo "  --cons             Use consensus mode        "
	echo "  --diss             Use dissensus mode        "
	exit 0;
...
if [ "${VERBOSE}" != "" ]; then
    ...
    echo "--------------------------------(Custom)----"
    echo "                                            "
    echo "OPT_MODE_PROMPT = [${OPT_MODE_PROMPT}]       "
...
#------------------------------------------------------------------
# Part 6.5 Launch the intruder
INTRUDER_IX=$((VAMT+1))

IVARGS="$VARGS --mport=900${INTRUDER_IX}  --pshare=920${INTRUDER_IX} "
VNAME="intruder"

IVARGS+=" --vname=${VNAME} "
vecho "About to launch intruder vehicle: $IVARGS"
./launch_intruder.sh $IVARGS	

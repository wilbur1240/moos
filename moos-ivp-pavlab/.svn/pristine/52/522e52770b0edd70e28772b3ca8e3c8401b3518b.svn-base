...
# Unrep configs
LEAD_VEHICLE="ben"
FOLLOWER_VEHICLE="abe"
I_AM_LEAD="no"  # Default, determined by vehicle name at launch
...
for ARGI; do
	...
	echo "  --loc=north   (or south)                       "
	echo "    Which location to use (north or south)       "
	echo "  --aux=<192.168.1.16:9250> ip and port of       "
	echo "    auxilary computer for reachabilty analysis   "
	echo "  --dctrl                                        "
	echo "    Launch the iSerial app for distrubance cntrl "
	echo "  --2V                                           "
	echo "    Two vehicles are used, wait for the other to "
	echo "    be ready before starting                     "
	echo "                                                 "
	...
	MOOS_PORT="${ARGI#--mport=*}"
        ...
        MAXSPD="${ARGI#--maxspd=*}"
    elif [ "${ARGI:0:6}" = "--loc=" ]; then
        LOC="${ARGI#--loc=*}"
    elif [ "${ARGI:0:6}" = "--aux=" ]; then
	AUX_SHARE_CONFIG="${ARGI#--aux=*}"
	SHARE_AUX="yes"
    elif [ "${ARGI:0:7}" = "--dctrl" ]; then
	DIST_CTRL="yes"
    elif [ "${ARGI:0:4}" = "--2V" ]; then
	OTHER_VEHICLE="yes"
...
nsplug meta_vehicle.moos targ_$VNAME.moos $NSFLAGS WARP=$TIME_WARP \
       ...
       SHARE_AUX=$SHARE_AUX         AUX_SHARE_CONFIG=$AUX_SHARE_CONFIG \
       DIST_CTRL=$DIST_CTRL

nsplug meta_vehicle.bhv targ_$VNAME.bhv $NSFLAGS VNAME=$VNAME  \
       SPEED=$SPEED                 START_POS=$START_POS       \
       LOC=$LOC                     OTHER_VEHICLE=$OTHER_VEHICLE \
       LEAD_VEHICLE=$LEAD_VEHICLE   FOLLOWER_VEHICLE=$FOLLOWER_VEHICLE \
       I_AM_LEAD=$I_AM_LEAD

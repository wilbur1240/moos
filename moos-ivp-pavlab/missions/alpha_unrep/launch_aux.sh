...
# default vehicle sharing options. 
LOCAL_IP="127.0.0.1"
SHARE_VEHICLE_NAME="vabe"
SHARE_TO_VEHICLE="no"
VEHICLE_SHARE_CONFIG="$LOCAL_IP:9215"
...
for ARGI; do
	...
	MOOS_PORT="${ARGI#--mport=*}"
        ...
        SHARE_VEHICLE_NAME="abe"
	SHARE_TO_VEHICLE="yes"
    elif [ "${ARGI}" = "-B" ]; then
        SHARE_VEHICLE_NAME="ben"
	SHARE_TO_VEHICLE="yes"
    elif [ "${ARGI}" = "-M" ]; then
        SHARE_VEHICLE_NAME="max"
	SHARE_TO_VEHICLE="yes"
    elif [ "${ARGI}" = "-VA" ]; then
        SHARE_VEHICLE_NAME="vabe"
	SHARE_TO_VEHICLE="yes"
    elif [ "${ARGI}" = "-VB" ]; then
        SHARE_VEHICLE_NAME="vben"
	SHARE_TO_VEHICLE="yes"
    elif [ "${ARGI}" = "-VM" ]; then
        SHARE_VEHICLE_NAME="vmax"
	SHARE_TO_VEHICLE="yes"
...
#--------------------------------------------------------------
#  Part 3b: Use INDEX for Other Settings
#--------------------------------------------------------------
...
#--------------------------------------------------------------
#  Part 3c: If sharing to a vehicle, determine the settings
#  for pShare
#--------------------------------------------------------------
VEHICLE_SHARE_CONFIG="$SHARE_VEHICLE_IP.9214"

if [ "${SHARE_VEHICLE_NAME}" = "abe" ]; then
    VEHICLE_SHARE_CONFIG="192.168.14.100:9214"
elif [ "${SHARE_VEHICLE_NAME}" = "ben" ]; then
    VEHICLE_SHARE_CONFIG="192.168.15.100:9215"
elif [ "${SHARE_VEHICLE_NAME}" = "max" ]; then
    VEHICLE_SHARE_CONFIG="192.168.20.100:9220"
elif [ "${SHARE_VEHICLE_NAME}" = "vabe" ]; then
    # use the default ip address, it is likely localhost
    VEHICLE_SHARE_CONFIG="$LOCAL_IP:9214"
elif [ "${SHARE_VEHICLE_NAME}" = "vben" ]; then
    # use the default ip address, it is likely localhost
    VEHICLE_SHARE_CONFIG="$LOCAL_IP:9215"
elif [ "${SHARE_VEHICLE_NAME}" = "vmax" ]; then
    # use the default ip address, it is likely localhost
    VEHICLE_SHARE_CONFIG="$LOCAL_IP:9220"
fi
...
nsplug meta_aux.moos targ_$VNAME.moos $NSFLAGS WARP=$TIME_WARP \
       ...
       SHARE_TO_VEHICLE=$SHARE_TO_VEHICLE                      \
       VEHICLE_SHARE_CONFIG=$VEHICLE_SHARE_CONFIG              \
       SHARE_VEHICLE_NAME=$SHARE_VEHICLE_NAME        

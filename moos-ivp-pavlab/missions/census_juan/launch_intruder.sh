...
COLOR="yellow"
XMODE="M300"
START_POS="x=85673,y=8089,heading=90"  
STOCK_SPD="1.4"
...
for ARGI; do
	...
	echo "                                                 "
	echo "  --vx1=<X,Y>            vx1 of LegRun           " 
	echo "  --vx2=<X,Y>            vx2 of LegRun           " 
	echo "  --start_pos=<X,Y,Hdg>  Sim start pos/hdg       "
	echo "  --stock_spd=<m/s>      Default vehicle speed   "
	echo "                                                 "
	exit 0;
    ...
    # Special case convenience hack for presidio
    elif [ "${ARGI}" = "--37" -o "${ARGI}" = "-37" ]; then
...
nsplug meta_intruder.moos targ_$VNAME.moos $NSFLAGS WARP=$TIME_WARP \
...
nsplug meta_intruder.bhv targ_$VNAME.bhv $NSFLAGS         \

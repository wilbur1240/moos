...
for ARGI; do
	...
	echo "  --advers                                       " 
	echo "    This vehicle is an adversary                 "
	echo "  --circle_pattern                               " 
	echo "    If this vehicle is an adversary, it will do  "
	echo "    the circle pattern.                          "
	echo "                                                 "
	echo "  --east, -e                                     " 
    ...
    elif [ "${ARGI}" = "--advers" ]; then
        ADVERSARY="yes"
	
    elif [ "${ARGI}" = "--circle_pattern" ]; then
        PATTERN="circle"
...
#--------------------------------------------------------------
#  Part 3a: Check for VNAME. Use INDEX for Other Settings
#--------------------------------------------------------------
if [ -z $VNAME ]; then
    if [ "${XMODE}" = "SIM" ]; then
	VNAME="abe"
...
#--------------------------------------------------------------
#  Part 3b: Apply INDEX to port vals. If Heron, set vals by vname. 
#--------------------------------------------------------------
MOOS_PORT=`expr $INDEX + 9000`
PSHARE_PORT=`expr $INDEX + 9200`
...
if [ ${ADVERSARY} = "no" ] ; then
    ...
    nsplug meta_vehicle_intruder.moos targ_$VNAME.moos $NSFLAGS WARP=$TIME_WARP \
       PSHARE_PORT=$PSHARE_PORT     VNAME=$VNAME               \
       START_POS=$START_POS         SHORE_IP=$SHORE_IP         \
       SHORE_PSHARE=$SHORE_PSHARE   MOOS_PORT=$MOOS_PORT       \
       IP_ADDR=$IP_ADDR             REGION=$REGION             \
    ...
    nsplug meta_vehicle_intruder.bhv targ_$VNAME.bhv $NSFLAGS VNAME=$VNAME  \
       SPEED=$SPEED    ZONE=$ZONE                              \

...
CPAEventNew CPAMonitor::getEvent(unsigned int ix) const
{
  CPAEventNew null_event;
  if(ix >= m_events.size())
    return(null_event);
  return(m_events[ix]);
}
...
bool CPAMonitor::examineAndReport(string vname, string contact)
{
  ...
  if(prev_closing && !now_closing) {
    ...
    if(cpa_dist <= m_report_range) {
      CPAEventNew event(vname, contact, m_map_cpa_records[vname].getSpec(), m_map_cpa_records[contact].getSpec(), cpa_dist);
...
bool CPAMonitor::updatePairRangeAndRate(string vname, string contact)
{
  ...
  if(dist_prev > dist) {
    ...
    m_map_cpa_records[vname] = m_map_vrecords[vname].front();
    m_map_cpa_records[contact] = m_map_vrecords[contact].front();
  }
...
string CPAMonitor::getLastReport(string vname)
{
  // Sanity check - make sure we have node records for each
  if(!m_map_vrecords.count(vname))
    return("NONE");
  ...
  double osx  = os_record.getX();
  double osy  = os_record.getY();
  string report=os_record.getSpec();
  return report;
}

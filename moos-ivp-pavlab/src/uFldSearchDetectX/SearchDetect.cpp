...
bool SearchDetect::OnNewMail(MOOSMSG_LIST &NewMail)
{
  ...
  for(p=NewMail.begin(); p!=NewMail.end(); p++) {
    ...
    if(key == "NODE_REPORT") 
      ...
      handled = setBooleanOnString(m_mission_start, sval);
...
//   Purpose: Parses out the detection range increments
//   Example: detect_range = 1:20:20 
//            detect_range = start:end:steps
